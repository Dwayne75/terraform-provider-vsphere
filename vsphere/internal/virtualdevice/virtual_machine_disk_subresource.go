...
func DiskSubresourceSchema() map[string]*schema.Schema {
	s := map[string]*schema.Schema{
		...
		"unit_number": {
			...
			Optional:     true,
			Default:      0,
			Description:  "The unique device number for this disk. This number determines where on the virtual bus this device will be attached.",
			ValidateFunc: validation.IntAtLeast(0),
		},
		"keep_on_remove": {
			Type:        schema.TypeBool,
...
func (r *DiskSubresource) assignDisk(l object.VirtualDeviceList, disk *types.VirtualDisk) (types.BaseVirtualController, error) {
	...
	case "scsi":
		...
		// The number of usable slots per controller depends on the SCSI
...
func (r *Subresource) findControllerInfo(l object.VirtualDeviceList, disk *types.VirtualDisk) (int, types.BaseVirtualController, error) {
	...
	case types.BaseVirtualSCSIController:
		...
		// Use controller-type-aware multiplier for unit number encoding.
		// PVSCSI supports 63 usable units per controller, LSI Logic/SAS 15.
		scsiType := r.rdd.Get("scsi_type").(string)
